...
export class KawaseBlurPass extends Pass {
  ...
  setBlurs(blurOffset, blurCount) {
    ...
    for (const [i, k] of kernels.entries()) {
      const uOffset = new Vector2().setScalar(0.5 + k).divide(res);
  ...
  render(renderer, writeBuffer, readBuffer) {
    ...
    this._internalComposer.passes[
      this._internalComposer.passes.length - 1
    ].shouldRenderToSreen = this.renderToScreen;
